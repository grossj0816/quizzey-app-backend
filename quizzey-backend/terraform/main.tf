...
# fetch all questions by setId -----------------------------------------------------------------
resource "aws_lambda_permission" "get_questions_by_sid_lambda_perm" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.get_questions_by_sid_lambda.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.quizzey-api-gateway.execution_arn}/*/*"
}

resource "aws_cloudwatch_log_group" "get_questions_by_sid_lambda" {
  name              = "/aws/lambda/${aws_lambda_function.get_questions_by_sid_lambda.function_name}"
  retention_in_days = 30
  skip_destroy      = false
}

resource "aws_lambda_function" "get_questions_by_sid_lambda" {
  depends_on       = [aws_s3_object.quizzey-object]
  s3_bucket        = "tu-api-lambda-deploys"
  s3_key           = "quizzey_app/lambdas.zip"
...
# create questions -----------------------------------------------------------------
resource "aws_lambda_permission" "create_questions_lambda_perm" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.create_questions_lambda.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.quizzey-api-gateway.execution_arn}/*/*"
}

resource "aws_cloudwatch_log_group" "create_questions_lambda" {
  name              = "/aws/lambda/${aws_lambda_function.create_questions_lambda.function_name}"
  retention_in_days = 30
  skip_destroy      = false
}

resource "aws_lambda_function" "create_questions_lambda" {
  depends_on       = [aws_s3_object.quizzey-object]
  s3_bucket        = "tu-api-lambda-deploys"
  s3_key           = "quizzey_app/lambdas.zip"
...
# update questions -----------------------------------------------------------------
resource "aws_lambda_permission" "update_questions_lambda_perm" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.update_questions_lambda.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.quizzey-api-gateway.execution_arn}/*/*"
}

resource "aws_cloudwatch_log_group" "update_questions_lambda" {
  name              = "/aws/lambda/${aws_lambda_function.update_questions_lambda.function_name}"
  retention_in_days = 30
  skip_destroy      = false
}

resource "aws_lambda_function" "update_questions_lambda" {
  depends_on       = [aws_s3_object.quizzey-object]
  s3_bucket        = "tu-api-lambda-deploys"
  s3_key           = "quizzey_app/lambdas.zip"
...
resource "aws_lambda_permission" "cors_questions_lambda_perm" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.cors_question_lambda.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.quizzey-api-gateway.execution_arn}/*/*"
}
