...
resource "aws_lambda_permission" "courses_get_lambda_perm" {
    ...
    # source_arn    = "${aws_api_gateway_rest_api.}"
}
...
# update questions -------------------------------------------------------------------
resource "aws_lambda_permission" "questions_update_lambda_perm" {
    statement_id  = "AllowExecutionFromAPIGateway"
    action        = "lambda:InvokeFunction"
    function_name = "${aws_lambda_function.questions_update_lambda.function_name}" 
    principal     = "apigateway.amazonaws.com"
    # source_arn    = "${aws_api_gateway_rest_api.}"
}


resource "aws_lambda_function" "questions_update_lambda" {
